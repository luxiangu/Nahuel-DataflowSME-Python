    ...
    def expand(self, input):
        ...
        if (not self.args.input == None) and (not self.args.input == "") :
            return (input
                ...
                | ParDo(ParseEventFn())
                | beam.Map(lambda element: TimestampedValue(element, element[self.TIMESTAMP_ATTRIBUTE]))
            )
        else:
            return (input
                    ...
                    | ReadFromPubSub(self.args.topic, timestamp_attribute=self.TIMESTAMP_ATTRIBUTE)
